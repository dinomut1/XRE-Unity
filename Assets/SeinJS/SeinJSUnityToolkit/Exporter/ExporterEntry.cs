...
namespace SeinJS
{
    public class ExporterEntry
    {
        ...
        private Dictionary<int, Dictionary<int, MaterialId>> _lightMapMat2ID = new Dictionary<int, Dictionary<int, MaterialId>>();
        ...
        public EntryBufferView CreateByteBufferView(string name, int size, int stride)
        {
            ...
            //bufferView.view.Target = BufferViewTarget.
        ...
        public NodeId SaveNode(Transform tr, bool savingColliders = false)
        {
            if (root.Nodes == null)
            {
            ...
            Regex colliderPattern = new Regex(".*__COLLIDER__");
            //check for singleton collider objects
            //check if collider parent

            var colliderParent = tr.GetComponent<ColliderParent>();
            ...
            {
                if (savingColliders && colliderPattern.IsMatch(tr.name))
                {
                    ...
                    if (collider != null)
                    {
                        ...
                        extras = new JProperty("extras", new JObject(
                            new JProperty("type", colliderType)//,
                                                               //new JProperty("data", "physics")
                        ));
        ...
        public Pair<MeshId, bool>SaveMesh(UnityEngine.Mesh mesh, Renderer renderer)
        {
            ...
            /*
            if(hasLightmap) //we still need to perform the lightmap compositing, so we must add the uv0s back to the mesh
            {
                mesh.uv = tmpUV;
        ...
        private Dictionary<string, AccessorId> GenerateAttributes(UnityEngine.Mesh mesh, bool bakedLightmaps = false)
        {
            ...
            {
                //_stride = ((_stride % 4) + 1) * 3;
                return CreateByteBufferView(mesh.name + "-" + suffix, _stride * mesh.vertexCount, _stride);
            };

            int offset = 0;// mesh.vertices.Length % 4;

            int stride = 3 * 4;
            attrs.Add("POSITION", PackAttrToBuffer(BufView("POSITION", stride), mesh.vertices, offset, (Vector3[] data, int i) => { return Utils.ConvertVector3LeftToRightHandedness(ref data[i]); }));
            //offset += stride * mesh.vertexCount;

            if (mesh.normals.Length > 0 && !ExporterSettings.Export.unlit)
            {
                stride = 3 * 4;
                attrs.Add("NORMAL", PackAttrToBuffer(BufView("NORMAL", stride), mesh.normals, offset, (Vector3[] data, int i) => { return Utils.ConvertVector3LeftToRightHandedness(ref data[i]); }));
                //offset += stride * mesh.vertexCount;
            }
            ...
            {
                stride = 4 * 4;
                attrs.Add("COLOR_0", PackAttrToBuffer(BufView("COLOR_0", stride), mesh.colors, offset));
                //offset += stride * mesh.vertexCount;
            }
            ...
            {
                stride = 2 * 4;
                attrs.Add("TEXCOORD_0", PackAttrToBuffer(BufView("TEXCOORD_0", stride), mesh.uv, offset));
                //offset += stride * mesh.vertexCount;
            }
            ...
            {
                stride = 2 * 4;
                attrs.Add("TEXCOORD_1", PackAttrToBuffer(BufView("TEXCOORD_1", stride), mesh.uv2, offset));
                //offset += stride * mesh.vertexCount;
            }
            ...
            {
                stride = 4 * 4;
                attrs.Add("TANGENT", PackAttrToBuffer(BufView("TANGENT", stride), mesh.tangents, offset, (Vector4[] data, int i) => { return Utils.ConvertVector4LeftToRightHandedness(ref data[i]); }));
                //offset += stride * mesh.vertexCount;
            }
            ...
            {
                stride = 2 * 4;
                attrs.Add("JOINTS_0", PackAttrToBufferShort(BufView("JOINTS_0", stride), mesh.boneWeights, offset));
                //offset += stride * mesh.vertexCount;
                stride = 4 * 4;
                attrs.Add("WEIGHTS_0", PackAttrToBuffer(BufView("WEIGHTS_0", stride), ExporterUtils.BoneWeightToWeightVec4(mesh.boneWeights), offset));
                //offset += stride * mesh.vertexCount;
            }

            foreach(var attr in attrs)
            {
                attr.Value.Value.Name += mesh.name + "-" + attr.Key;
            }
        ...
        private void SaveIndices(UnityEngine.Mesh mesh, MeshPrimitive primitive, int i, ref EntryBufferView bufferView)
        {
            ...
            int byteOffset = primitive.Indices.Value.ByteOffset;
        ...
        public MaterialId SaveNormalMaterial(UnityEngine.Material material, UnityEngine.Renderer renderer = null)
        {
            ...
            if (!addLightMappedEntry && _material2ID.ContainsKey(mid))
            {
			...
			var mat = ExporterUtils.ConvertMaterial(material, this, renderer);
			root.Materials.Add(mat);

			var id = new MaterialId { Id = root.Materials.Count - 1, Root = root };
            

            if(addLightMappedEntry)
            {
                _lightMapMat2ID[mid][rid] = id;
            }
            else
            {
        ...
        public void Finish()
        {
            ...
            //root.Buffers = new List<GLTF.Schema.Buffer>();
            using (var binWriter = new BinaryWriter(outputFile)) {
                ...
                foreach (var bufferView in _bufferViews)
                {
                    ...
                    //bufferId = new BufferId { Id = bufferhttps://github.com/ubisoft/mixer.gitId.Id + 1, Root = root };
                }
